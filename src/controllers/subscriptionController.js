...
// customer or admin: permanently cancel a subscription (distinct from pause,
// which can be resumed). Cancelled subscriptions are excluded from the daily
// order generator automatically since it only queries active: true.
exports.cancel = async (req, res) => {
  ...
  s.cancelled = true;
  await s.save();
  emit.subscriptionChanged(s); // -> drops off admin's active list and stops generating future orders
  res.json({ ok: true, subscription: s });
};

// admin only: manually trigger today's (or a given date's) subscription->order
// generation, for testing or to re-run after fixing a misconfigured plan
// without waiting for the nightly cron.
exports.runDailyGeneration = async (req, res) => {
  const { generateOrdersForDate } = require('../utils/generateSubscriptionOrders');
  const date = req.body.date ? new Date(req.body.date) : new Date();
  const result = await generateOrdersForDate(date);
